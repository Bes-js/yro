// Type definitions for yro 2.7.1
// Definitions by: João Portela https://www.github.com/jportela

// Exported Methods

/**
 * Either connects to a running yro daemon (“God”) or launches and daemonizes one.
 * Once launched, the yro process will keep running after the script exits.
 * @param errback - Called when finished connecting to or launching the yro daemon process.
 */
 ...
 * yro will not be run as a daemon and will die when the related script exits.
 * By default, yro stays alive after your script exits.
 * If yro is already running, your script will link to the existing daemon but will die once your process exits.
 * @param errback - Called when finished connecting to or launching the yro daemon process.
 */
export function connect(noDaemonMode:boolean, errback: ErrCallback): void;

/**
 * Starts a script that will be managed by yro.
 * @param options - Options
 * @param errback - An errback called when the script has been started.
 * The proc parameter will be a yro process object.
 */
...
/**
 * Disconnects from the yro daemon.
 */
export function disconnect(): void;

/**
 * Stops a process but leaves the process meta-data in yro’s list
 * @param process - Can either be the name as given in the yro.start options,
...
/**
 * Stops the process and removes it from yro’s list.
 ...
 * @param options.updateEnv - (Default: false) If true is passed in, yro will reload it’s
...
/**
 * Kills the yro daemon (same as yro kill). Note that when the daemon is killed, all its
...
/**
 * Gets the list of running processes being managed by yro.
 * @param errback
 */
...
export interface ProcessDescription {
  ...
  pid?: number;
  /**
   * The pid for the yro God daemon process.
   */
  pm_id?: number;
  monit?: Monit;
  /**
...
export interface StartOptions {
  ...
  cwd?: string;
  /**
   * (Default: “~/.yro/logs/app_name-out.log”) The path to a file to append stdout output to.
   * Can be the same file as error.
   */
  output?: string;
  /**
   * (Default: “~/.yro/logs/app_name-error.err”) The path to a file to append stderr output to. Can be the same file as output.
   */
  error?: string;
  /**
  ...
  /**
   * Default: “~/.yro/logs/~/.yro/pids/app_name-id.pid”)
   * The path to a file to write the pid of the started process. The file will be overwritten.
   * Note that the file is not used in any way by yro and so the user is free to manipulate or
  ...
  max_restarts?: number;
  /**
   * If sets and script’s memory usage goes about the configured number, yro restarts the script.
  ...
  time?: boolean;
  /**
   * This will make yro listen for that event. In your application you will need to add process.send('ready');
   ...
   * ‘test.js’ started via yro, normally you would have 4 stdout log files and 4 stderr log files,
  ...
  /**
   * (Default: false) By default, yro will only start a script if that script isn’t
   * already running (a script is a path to an application, not the name of an application
   * already running). If force is set to true, yro will start a new instance of that script.
   */
  force?: boolean;
  ignore_watch?: string[];
  cron?: any;
...
interface ReloadOptions {
  /**
   * (Default: false) If true is passed in, yro will reload it’s environment from process.env
