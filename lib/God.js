...
var debug         = require('debug')('yro:god');
...
God.init = function() {
  ...
  this.configuration = Configuration.getSync('YRO')
...
God.writeExitSeparator = function(yro_env, code, signal) {
  try {
    var exit_sep = `[YRO][${new Date().toISOString()}] app exited`
    if (code)
    ...
    if (yro_env.pm_out_log_path)
      fs.writeFileSync(yro_env.pm_out_log_path, exit_sep)
    if (yro_env.pm_err_log_path)
      fs.writeFileSync(yro_env.pm_err_log_path, exit_sep)
    if (yro_env.pm_log_path)
      fs.writeFileSync(yro_env.pm_log_path, exit_sep)
  } catch(e) {
  }
}
...
God.executeApp = function executeApp(env, cb) {
  ...
  if (env_copy['autostart'] === false) {
    var clu = {yro_env: env_copy, process: {pid: 0}};
  ...
  var readyCb = function ready(proc) {
    ...
    proc.yro_env.version = Utility.findPackageVersion(proc.yro_env.pm_exec_path || proc.yro_env.cwd);
    // If vizion enabled run versioning retrieval system
    if (cst.ENABLE_GIT_PARSING === true &&
        proc.yro_env.vizion !== false && proc.yro_env.vizion !== "false") {
      God.finalizeProcedure(proc);
    }
    else
      God.notify('online', proc);

    if (proc.yro_env.status !== cst.ERRORED_STATUS)
      proc.yro_env.status = cst.ONLINE_STATUS

    console.log(`App [${proc.yro_env.name}:${proc.yro_env.pm_id}] online`);
    if (cb) cb(null, proc);
  }
  ...
  if (env_copy.exec_mode === 'cluster_mode') {
    ...
    God.nodeApp(env_copy, function nodeApp(err, clu) {
      ...
      var old_env = God.clusters_db[clu.yro_env.pm_id];

      if (old_env) {
        old_env = null;
        God.clusters_db[clu.yro_env.pm_id] = null;
      }

      God.clusters_db[clu.yro_env.pm_id] = clu;
      ...
      if (cst.IS_BUN) {
        ...
        var a = setTimeout(() => {
          if (clu.yro_env)
            God.clusters_db[clu.yro_env.pm_id].state = 'online'
          return readyCb(clu)
        }, 500)
      }
      ...
      clu.once('disconnect', function() {
        ...
        console.log('App name:%s id:%s disconnected', clu.yro_env.name, clu.yro_env.pm_id);
      });
      ...
      clu.once('exit', function cluExit(code, signal) {
        ...
        //God.writeExitSeparator(clu.yro_env, code, signal)
      ...
      return clu.once('online', function () {
        ...
        var ready_timeout = setTimeout(function() {
          ...
          return readyCb(clu)
        }, clu.yro_env.listen_timeout || cst.GRACEFUL_LISTEN_TIMEOUT);

        var listener = function (packet) {
          if (packet.raw === 'ready' &&
              packet.process.name === clu.yro_env.name &&
              packet.process.pm_id === clu.yro_env.pm_id) {
...
God.handleExit = function handleExit(clu, exit_code, kill_signal) {
  console.log(`App [${clu.yro_env.name}:${clu.yro_env.pm_id}] exited with code [${exit_code}] via signal [${kill_signal || 'SIGINT'}]`)

  var proc = this.clusters_db[clu.yro_env.pm_id];

  if (!proc) {
    console.error('Process undefined ? with process id ', clu.yro_env.pm_id);
    return false;
  }

  var stopExitCodes = proc.yro_env.stop_exit_codes !== undefined && proc.yro_env.stop_exit_codes !== null ? proc.yro_env.stop_exit_codes : [];
  if (!Array.isArray(stopExitCodes)) {
    stopExitCodes = [stopExitCodes];
  }

  var stopping = (proc.yro_env.status == cst.STOPPING_STATUS
                  || proc.yro_env.status == cst.STOPPED_STATUS
                  || proc.yro_env.status == cst.ERRORED_STATUS)
      || (proc.yro_env.autorestart === false || proc.yro_env.autorestart === "false")
  ...
  if (proc.yro_env.axm_actions) proc.yro_env.axm_actions = [];
  if (proc.yro_env.axm_monitor) proc.yro_env.axm_monitor = {};

  if (proc.yro_env.status != cst.ERRORED_STATUS &&
      proc.yro_env.status != cst.STOPPING_STATUS)
    proc.yro_env.status = cst.STOPPED_STATUS;

  if (proc.yro_env.pm_id.toString().indexOf('_old_') !== 0) {
    try {
      fs.unlinkSync(proc.yro_env.pm_pid_path);
    } catch (e) {
  ...
  var min_uptime = typeof(proc.yro_env.min_uptime) !== 'undefined' ? proc.yro_env.min_uptime : 1000;
  var max_restarts = typeof(proc.yro_env.max_restarts) !== 'undefined' ? proc.yro_env.max_restarts : 16;

  if ((Date.now() - proc.yro_env.created_at) < (min_uptime * max_restarts)) {
    if ((Date.now() - proc.yro_env.pm_uptime) < min_uptime) {
      // Increment unstable restart
      proc.yro_env.unstable_restarts += 1;
    }
  }


  if (proc.yro_env.unstable_restarts >= max_restarts) {
    ...
    console.log('Script %s had too many unstable restarts (%d). Stopped. %j',
      proc.yro_env.pm_exec_path,
      proc.yro_env.unstable_restarts,
      proc.yro_env.status);

    God.notify('restart overlimit', proc);

    proc.yro_env.unstable_restarts = 0;
    proc.yro_env.created_at = null;
    overlimit = true;
  }

  if (typeof(exit_code) !== 'undefined') proc.yro_env.exit_code = exit_code;

  God.notify('exit', proc);

  if (God.YRO_being_killed) {
    //console.log('[HandleExit] YRO is being killed, stopping restart procedure...');
    return false;
  }

  var restart_delay = 0;

  if (proc.yro_env.restart_delay !== undefined &&
      !isNaN(parseInt(proc.yro_env.restart_delay))) {
    proc.yro_env.status = cst.WAITING_RESTART;
    restart_delay = parseInt(proc.yro_env.restart_delay);
  }

  if (proc.yro_env.exp_backoff_restart_delay !== undefined &&
      !isNaN(parseInt(proc.yro_env.exp_backoff_restart_delay))) {
    proc.yro_env.status = cst.WAITING_RESTART;
    if (!proc.yro_env.prev_restart_delay) {
      proc.yro_env.prev_restart_delay = proc.yro_env.exp_backoff_restart_delay
      restart_delay = proc.yro_env.exp_backoff_restart_delay
    }
    else {
      proc.yro_env.prev_restart_delay = Math.floor(Math.min(15000, proc.yro_env.prev_restart_delay * 1.5))
      restart_delay = proc.yro_env.prev_restart_delay
    }
    console.log(`App [${clu.yro_env.name}:${clu.yro_env.pm_id}] will restart in ${restart_delay}ms`)
  }

  if (!stopping && !overlimit) {
    //make this property unenumerable
    Object.defineProperty(proc.yro_env, 'restart_task', {configurable: true, writable: true});
    proc.yro_env.restart_task = setTimeout(function() {
      proc.yro_env.restart_time += 1;
      God.executeApp(proc.yro_env);
    }, restart_delay);
  }
...
God.finalizeProcedure = function finalizeProcedure(proc) {
  var last_path    = '';
  var current_path = proc.yro_env.cwd || path.dirname(proc.yro_env.pm_exec_path);
  var proc_id      = proc.yro_env.pm_id;

  proc.yro_env.version = Utility.findPackageVersion(proc.yro_env.pm_exec_path || proc.yro_env.cwd);

  if (proc.yro_env.vizion_running === true) {
    debug('Vizion is already running for proc id: %d, skipping this round', proc_id);
    return God.notify('online', proc);
  }
  proc.yro_env.vizion_running = true;
  ...
  vizion.analyze({folder : current_path}, function recur_path(err, meta){
    ...
    proc.yro_env.vizion_running = false;

    if (!err) {
      proc.yro_env.versioning = meta;
      proc.yro_env.versioning.repo_path = current_path;
      God.notify('online', proc);
    }
    else if (err && current_path === last_path) {
      proc.yro_env.versioning = null;
      God.notify('online', proc);
    }
    else {
...
God.injectVariables = function injectVariables (env, cb) {
  ...
  var instanceKey = process.env.YRO_PROCESS_INSTANCE_VAR || env.instance_var;
  ...
  var instances = Object.keys(God.clusters_db)
    .map(function (procId) {
      ...
      return proc.yro_env.name === env.name &&
        typeof proc.yro_env[instanceKey] !== 'undefined';
    }).map(function (proc) {
      return proc.yro_env[instanceKey];
    }).sort(function (a, b) {
      return b - a;
    });
  ...
  if (env.increment_var) {
    var lastIncrement = Object.keys(God.clusters_db)
      .map(function (procId) {
        ...
        return proc.yro_env.name === env.name &&
          typeof proc.yro_env[env.increment_var] !== 'undefined';
      }).map(function (proc) {
        return Number(proc.yro_env[env.increment_var]);
      }).sort(function (a, b) {
        return b - a;
      })[0];
