...
var debug          = require('debug')('yro:client');
...
var Client = module.exports = function(opts) {
  ...
  this.yro_home    = this.conf.YRO_ROOT_PATH;
  ...
  // Client depends to that to interact with YRO properly
  ...
  this.pingDaemon(function(daemonAlive) {
    ...
    if (that.daemon_mode === false) {
      ...
      var daemon = new Daemon({
        ...
        pid_file        : that.conf.YRO_PID_FILE_PATH,
        ignore_signals  : true
      });
    ...
    that.launchDaemon(function(err, child) {
      ...
      if (!process.env.YRO_DISCRETE_MODE)
        Common.printOut(that.conf.PREFIX_MSG + 'YRO Successfully daemonized');
...
// Init file structure of YRO_home
// This includes
// - YRO pid and log path
...
Client.prototype.initFileStructure = function (opts) {
  ...
  if (!fs.existsSync(opts.YRO_MODULE_CONF_FILE)) {
    try {
      fs.writeFileSync(opts.YRO_MODULE_CONF_FILE, "{}");
    } catch (e) {
      console.error(e.stack || e);
    }
  }
  ...
  if (process.env.YRO_DISCRETE_MODE) {
    try {
      fs.writeFileSync(path.join(opts.YRO_HOME, 'touch'), Date.now().toString());
    } catch(e) {
      debug(e.stack || e);
    }
  }

  if (!process.env.YRO_PROGRAMMATIC && !fs.existsSync(path.join(opts.YRO_HOME, 'touch'))) {
    ...
    var dt = fs.readFileSync(path.join(__dirname, opts.YRO_BANNER));
...
Client.prototype.launchDaemon = function(opts, cb) {
  ...
  //   // Redirect YRO internal err and out to STDERR STDOUT when running with Travis
  //   out = 1;
  //   err = 2;
  // }
  // else {
  out = fs.openSync(that.conf.YRO_LOG_FILE_PATH, 'a'),
  err = fs.openSync(that.conf.YRO_LOG_FILE_PATH, 'a');
  //}
  ...
  /**
   * Add node [arguments] depending on YRO_NODE_OPTIONS env variable
   */
  if (process.env.YRO_NODE_OPTIONS)
    node_args = node_args.concat(process.env.YRO_NODE_OPTIONS.split(' '));
  node_args.push(ClientJS);

  if (!process.env.YRO_DISCRETE_MODE)
    Common.printOut(that.conf.PREFIX_MSG + 'Spawning YRO daemon with yro_home=' + this.yro_home);
  ...
  child.once('message', function(msg) {
    debug('YRO daemon launched with return message: ', msg);
    child.removeListener('error', onError);
    child.disconnect();
    if (opts && opts.interactor == false)
      return cb(null, child);

    if (process.env.YRO_NO_INTERACTION == 'true')
      return cb(null, child);

    /**
     * Here the Keymetrics agent is launched automaticcaly if
     * it has been already configured before (via YRO link)
     */
    KMDaemon.launchAndInteract(that.conf, {
      machine_name : that.machine_name,
      public_key   : that.public_key,
...
Client.prototype.pingDaemon = function pingDaemon(cb) {
  ...
  debug('[PING YRO] Trying to connect to server');
 ...
 * Once he's connected it trigger the command parsing (on ./bin/YRO file, at the end)
 * @method launchRPC
 * @params {function} [cb]
 * @return
 */
...
Client.prototype.disconnectRPC = function disconnectRPC(cb) {
  ...
  if (!this.client_sock || !this.client_sock.close) {
    ...
    return process.nextTick(function() {
      cb(new Error('SUB connection to YRO is not launched'));
    });
  }
  ...
  try {
    ...
    debug('Error while disconnecting RPC YRO', e.stack || e);
    return cb(e);
  }
  return false;
};
...
Client.prototype.executeRemote = function executeRemote(method, app_conf, fn) {
  ...
  debug('Calling daemon method YRO:%s on rpc socket:%s', method, this.rpc_socket_file);
...
Client.prototype.killDaemon = function killDaemon(fn) {
  ...
  if (process.platform !== 'win32' && process.platform !== 'win64') {
    process.once('SIGQUIT', function() {
      debug('Received SIGQUIT from YRO daemon');
...
Client.prototype.getAllProcessIdWithoutModules = function(cb) {
  ...
  this.executeRemote('getMonitorData', {}, function(err, procs) {
    ...
    var proc_ids = procs
        .filter(proc => !proc.yro_env.pmx_module)
        .map(proc => proc.pm_id)
...
Client.prototype.getProcessIdByName = function(name, force_all, cb) {
  ...
  this.executeRemote('getMonitorData', {}, function(err, list) {
    ...
    list.forEach(function(proc) {
      if (proc.yro_env.name == name || proc.yro_env.pm_exec_path == path.resolve(name)) {
...
Client.prototype.getProcessIdsByNamespace = function(namespace, force_all, cb) {
  ...
  this.executeRemote('getMonitorData', {}, function(err, list) {
    ...
    list.forEach(function(proc) {
      if (proc.yro_env.namespace == namespace) {
        found_proc.push(proc.pm_id);
        full_details[proc.pm_id] = proc;
      }
    });
...
Client.prototype.getProcessByName = function(name, cb) {
  ...
  this.executeRemote('getMonitorData', {}, function(err, list) {
    ...
    list.forEach(function(proc) {
      if (proc.yro_env.name == name ||
          proc.yro_env.pm_exec_path == path.resolve(name)) {
...
Client.prototype.getProcessByNameOrId = function (nameOrId, cb) {
  ...
  this.executeRemote('getMonitorData', {}, function (err, list) {
    ...
    list.forEach(function (proc) {
      if (proc.yro_env.name === nameOrId ||
        proc.yro_env.pm_exec_path === path.resolve(nameOrId) ||
        proc.pid === parseInt(nameOrId) ||
        proc.yro_env.pm_id === parseInt(nameOrId)) {
        foundProc.push(proc);
      }
    });
