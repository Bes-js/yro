...
/**
 * Check if dump file contains same apps that the one managed by YRO
 */
function checkIfProcessAreDumped(list) {
  try {
    ...
    var apps_running = list
        .filter(proc => proc.yro_env.pmx_module != true)
        .map(proc => proc.name)
    var diff = apps_dumped.filter(a => !apps_running.includes(a))
    if (diff.length > 0) {
      Common.warn(`Current process list is not synchronized with saved list. App ${chalk.bold(diff.join(' '))} differs. Type 'yro save' to synchronize.`)
    }
    else if (apps_dumped.length != apps_running.length) {
      Common.warn(`Current process list is not synchronized with saved list. Type 'yro save' to synchronize.`)
    }
  } catch(e) {
  }
}
...
/**
 * List Applications and Modules managed by YRO
 */
function listModulesAndAppsManaged(list, commander) {
  ...
  var id_width = Math.max(
    2 + (Math.max(...list.map((l) => String(l.yro_env.pm_id || 0).length)) || 0),
    4
  );
  ...
  var sortField = 'name', sortOrder = 'asc', sort,
      fields = {
        ...
        cpu: 'monit.cpu',
        memory: 'monit.memory',
        uptime: 'yro_env.pm_uptime',
        status: 'yro_env.status'
      }
  ...
  list.forEach(function(l) {
    ...
    if (l.yro_env.pm_id > proc_id) {
      proc_id = l.yro_env.pm_id
    }

    var mode = l.yro_env.exec_mode
    var status = l.yro_env.status
    var key = l.yro_env.pm_id
    key = chalk.bold.cyan(key)

    if (l.yro_env.axm_options) {
      var is_tracing_enabled = false

      if (l.yro_env.axm_options.tracing &&
          typeof(l.yro_env.axm_options.tracing) == 'boolean' &&
          l.yro_env.axm_options.tracing == true)
        is_tracing_enabled = true

      if (l.yro_env.axm_options.tracing &&
          l.yro_env.axm_options.tracing.enabled &&
          typeof(l.yro_env.axm_options.tracing.enabled) == 'boolean' &&
          l.yro_env.axm_options.tracing.enabled == true)
        is_tracing_enabled = true

      if (is_tracing_enabled == true)
        l.yro_env.name = chalk.green('☵')  + ' ' + l.yro_env.name

      if (l.yro_env._km_monitored)
        l.yro_env.name = chalk.bold.green('◉')  + ' ' + l.yro_env.name
    }

    if (l.yro_env.pmx_module == true) {
      if (l.yro_env.name == 'YRO-sysmonit') return
      // YRO ls for Modules
      obj[key] = []
      ...
      // Module version + PID
      if (!CONDENSED_MODE) {
        var pid = l.yro_env.axm_options.pid ? l.yro_env.axm_options.pid : l.pid
        obj[key].push(l.yro_env.version || 'N/A', pid)
      }
      ...
      // Restart
      if (!CONDENSED_MODE)
        obj[key].push(l.yro_env.restart_time ? l.yro_env.restart_time : 0)
      ...
      if (!CONDENSED_MODE) {
        ...
        if (l.yro_env.uid && typeof(l.yro_env.uid) == 'number') {
          // Resolve user id to username
          let users = Passwd.getUsers()
          Object.keys(users).forEach(function(username) {
            var user = users[username]
            if (user.userId == l.yro_env.uid) {
              l.yro_env.uid = user.username
            }
          })
        }
        obj[key].push(chalk.bold(l.yro_env.uid || l.yro_env.username))
      }

      UxHelpers.safe_push(module_table, obj)
    }
    else {
      // YRO ls for Applications
      obj[key] = []

      // YRO ID
      obj[key].push(l.yro_env.name)

      // Namespace
      if (!CONDENSED_MODE)
        obj[key].push(l.yro_env.namespace)

      // Version
      if (!CONDENSED_MODE)
        obj[key].push(l.yro_env.version)
      ...
      // Uptime
      if (!CONDENSED_MODE)
        obj[key].push((l.yro_env.pm_uptime && status == 'online') ? UxHelpers.timeSince(l.yro_env.pm_uptime) : 0)
      ...
      // Watch status
      if (!CONDENSED_MODE)
        obj[key].push(l.yro_env.watch ? chalk.green.bold('enabled') : chalk.gray('disabled'))
...
function miniMonitBar(sys_infos) {
  let sys_metrics = sys_infos.yro_env.axm_monitor
 ...
 * @method dispAsTable
 * @param {Object} list
 * @param {Object} system informations (via YRO sysmonit/YRO sysinfos)
 */
module.exports = function(list, commander) {
  var yro_conf = Configuration.getSync('yro')
  ...
  let sysmonit = list.filter(proc => proc.name == 'yro-sysmonit')
