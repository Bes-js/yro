...
module.exports = function(CLI) {
  /**
   * Get version of the daemonized YRO
   * @method getVersion
   * @callback cb
   */
  ...
  /**
   * Install YRO-sysmonit
   */
  CLI.prototype.launchSysMonitoring = function(cb) {
    if ((this.yro_configuration && this.yro_configuration.sysmonit != 'true') ||
        process.env.TRAVIS ||
        global.it === 'function' ||
        cst.IS_WINDOWS === true)
  ...
  CLI.prototype.env = function(app_id, cb) {
    ...
    this.Client.executeRemote('getMonitorData', {}, (err, list) => {
      list.forEach(l => {
        if (app_id == l.pm_id) {
          printed++
          var env = Common.safeExtend({}, l.yro_env)
  ...
  CLI.prototype.report = function() {
    ...
    that.Client.executeRemote('getReport', {}, function(err, report) {
      ...
      console.log()
      console.log('```')
      fmt.title('YRO report')
      fmt.field('Date', new Date());
      fmt.sep();

      if (report && !err) {
        fmt.title(chalk.bold.blue('Daemon'));
        fmt.field('yrod version', report.yro_version);
      ...
      fmt.field('local YRO', pkg.version);
      ...
      that.Client.executeRemote('getMonitorData', {}, function(err, list) {
        ...
        fmt.sep();
        fmt.title(chalk.bold.blue('YRO list'));
        ...
        Log.tail([{
          ...
          console.log(chalk.bold.green('Please copy/paste the above report in your issue on https://github.com/Unitech/YRO/issues'));
  ...
  CLI.prototype.serve = function (target_path, port, opts, commander, cb) {
    var that = this;
    var servePort = process.env.YRO_SERVE_PORT || port || 8080;
    var servePath = path.resolve(process.env.YRO_SERVE_PATH || target_path || '.');
    ...
    if (!opts.env)
      opts.env = {};
    opts.env.YRO_SERVE_PORT = servePort;
    opts.env.YRO_SERVE_PATH = servePath;
    opts.env.YRO_SERVE_SPA = opts.spa;
    if (opts.basicAuthUsername && opts.basicAuthPassword) {
      opts.env.YRO_SERVE_BASIC_AUTH = 'true';
      opts.env.YRO_SERVE_BASIC_AUTH_USERNAME = opts.basicAuthUsername;
      opts.env.YRO_SERVE_BASIC_AUTH_PASSWORD = opts.basicAuthPassword;
    }
    if (opts.monitor) {
      opts.env.YRO_SERVE_MONITOR = opts.monitor
    }
    opts.cwd = servePath;
  ...
  /**
   * Ping daemon - if YRO daemon not launched, it will launch it
   * @method ping
   */
   ...
   * to YRO
   * It is used for the new scoped YRO action system
   */
