 ...
 */
// YRO-htop
// Library who interacts with YRO to display processes resources in htop way
...
Monit.reset = function(msg) {
  ...
  this.multi.write('\x1B[32m⌬ YRO \x1B[39mmonitoring\x1B[96m (To go further check out https://app.YRO.io) \x1B[39m\n\n');
...
Monit.refresh = function(processes) {
  ...
  if(num !== this.num_bars) {
    ...
    for(var i = 0; i < num; i++) {
      ...
      if(this.bars[proc.pm_id] && proc.yro_env.status !== this.bars[proc.pm_id].status) {
...
Monit.addProcess = function(proc, i) {
  ...
  var process_name = proc.yro_env.name || p.basename(proc.yro_env.pm_exec_path);
  var status = proc.yro_env.status == 'online' ? chalk.green.bold('●') : chalk.red.bold('●');

  this.multi.write(' ' + status + ' ' + chalk.green.bold(process_name));
  this.multi.write('\n');
  this.multi.write('[' + proc.yro_env.pm_id + '] [' + proc.yro_env.exec_mode + ']\n');
...
Monit.updateBars = function(proc) {
  if (this.bars[proc.pm_id]) {
    if (proc.yro_env.status !== 'online' || proc.yro_env.status !== this.bars[proc.pm_id].status) {
      this.bars[proc.pm_id].cpu.percent(0, chalk.red(proc.yro_env.status));
      this.drawRatio(this.bars[proc.pm_id].memory, 0, chalk.red(proc.yro_env.status));
    } else if (!proc.monit) {
