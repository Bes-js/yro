...
function parseAndSwitch(file_content, main_file, opts) {
  ...
  lines[0] = 'FROM keymetrics/yro:' + opts.node_version;
  ...
  for (var i = 0; i < lines.length; i++) {
    ...
    if (['## DISTRIBUTION MODE', '## DEVELOPMENT MODE'].indexOf(line) > -1 ||
      ...
      if (mode == 'distribution') {
        ...
        lines[i + 3] = 'CMD ["yro-docker", "' + main_file + '", "--env", "production"]';
      }
      if (mode == 'development') {
        lines[i + 2] = 'CMD ["yro-dev", "' + main_file + '", "--env", "development"]';
      }
      break;
    }
  };
