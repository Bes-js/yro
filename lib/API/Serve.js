...
var debug = require('debug')('yro:serve');
...
var options = {
  port: process.env.YRO_SERVE_PORT || process.argv[3] || 8080,
  host: process.env.YRO_SERVE_HOST || process.argv[4] || '0.0.0.0',
  path: path.resolve(process.env.YRO_SERVE_PATH || process.argv[2] || '.'),
  spa: process.env.YRO_SERVE_SPA === 'true',
  homepage: process.env.YRO_SERVE_HOMEPAGE || '/index.html',
  basic_auth: process.env.YRO_SERVE_BASIC_AUTH === 'true' ? {
    username: process.env.YRO_SERVE_BASIC_AUTH_USERNAME,
    password: process.env.YRO_SERVE_BASIC_AUTH_PASSWORD
  } : null,
  monitor: process.env.YRO_SERVE_MONITOR
};
...
if (typeof options.monitor === 'string' && options.monitor !== '') {
  try {
    let fileContent = fs.readFileSync(path.join(process.env.YRO_HOME, 'agent.json5')).toString()
...
}(window,document,'https://apm.pm2.io/pm2-io-apm-browser.v1.js','yroReady','script'))

yroReady(function(apm) {
