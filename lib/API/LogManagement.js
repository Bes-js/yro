...
module.exports = function(CLI) {
  ...
  CLI.prototype.flush = function(api, cb) {
    ...
    if (!api) {
      Common.printOut(cst.PREFIX_MSG + 'Flushing ' + cst.YRO_LOG_FILE_PATH);
      fs.closeSync(fs.openSync(cst.YRO_LOG_FILE_PATH, 'w'));
    }
    ...
    that.Client.executeRemote('getMonitorData', {}, function(err, list) {
      ...
      list.forEach(function(l) {
        if (typeof api == 'undefined') {
          ...
          Common.printOut(cst.PREFIX_MSG + l.yro_env.pm_out_log_path);
          Common.printOut(cst.PREFIX_MSG + l.yro_env.pm_err_log_path);

          if (l.yro_env.pm_log_path) {
            Common.printOut(cst.PREFIX_MSG + l.yro_env.pm_log_path);
            fs.closeSync(fs.openSync(l.yro_env.pm_log_path, 'w'));
          }
          fs.closeSync(fs.openSync(l.yro_env.pm_out_log_path, 'w'));
          fs.closeSync(fs.openSync(l.yro_env.pm_err_log_path, 'w'));
        }
        else if (l.yro_env.pm_id == api || l.yro_env.name === api) {
          Common.printOut(cst.PREFIX_MSG + 'Flushing:');

          if (l.yro_env.pm_log_path && fs.existsSync(l.yro_env.pm_log_path)) {
            Common.printOut(cst.PREFIX_MSG + l.yro_env.pm_log_path);
            fs.closeSync(fs.openSync(l.yro_env.pm_log_path, 'w'));
          }

          if (l.yro_env.pm_out_log_path && fs.existsSync(l.yro_env.pm_out_log_path)) {
            Common.printOut(cst.PREFIX_MSG + l.yro_env.pm_out_log_path);
            fs.closeSync(fs.openSync(l.yro_env.pm_out_log_path, 'w'));
          }

          if (l.yro_env.pm_err_log_path && fs.existsSync(l.yro_env.pm_err_log_path)) {
            Common.printOut(cst.PREFIX_MSG + l.yro_env.pm_err_log_path);
            fs.closeSync(fs.openSync(l.yro_env.pm_err_log_path, 'w'));
          }
        }
  ...
  CLI.prototype.logrotate = function(opts, cb) {
    ...
    if (process.getuid() != 0) {
      return exec('whoami', function(err, stdout, stderr) {
        ...
        Common.printError(cst.PREFIX_MSG + chalk.gray('      sudo env PATH=$PATH:' + path.dirname(process.execPath) + ' yro logrotate -u ' + stdout.trim()));
    ...
    script = script.replace(/%HOME_PATH%/g, cst.YRO_ROOT_PATH)
      .replace(/%USER%/g, user);

    try {
      fs.writeFileSync('/etc/logrotate.d/yro-'+user, script);
    } catch (e) {
      console.error(e.stack || e);
    }

    Common.printOut(cst.PREFIX_MSG + 'Logrotate configuration added to /etc/logrotate.d/yro');
  ...
  CLI.prototype.streamLogs = function(id, lines, raw, timestamp, exclusive, highlight) {
    ...
    that.Client.executeRemote('getMonitorData', {}, function(err, list) {
      ...
      list.forEach(function(proc) {
        if (proc.yro_env && (id === 'all' ||
                             proc.yro_env.name == id ||
                             proc.yro_env.pm_id == id)) {
          if (proc.yro_env.pm_out_log_path && exclusive !== 'err')
            pushIfUnique({
              path     : proc.yro_env.pm_out_log_path,
              app_name :proc.yro_env.pm_id + '|' + proc.yro_env.name,
              type     : 'out'});
          if (proc.yro_env.pm_err_log_path && exclusive !== 'out')
            pushIfUnique({
              path     : proc.yro_env.pm_err_log_path,
              app_name : proc.yro_env.pm_id + '|' + proc.yro_env.name,
              type     : 'err'
            });
        } else if(proc.yro_env && proc.yro_env.namespace == id) {
          if(namespaceList.indexOf(proc.yro_env.name) === -1) {
            namespaceList.push(proc.yro_env.name)
          }
          if (proc.yro_env.pm_out_log_path && exclusive !== 'err')
            pushIfUnique({
              path     : proc.yro_env.pm_out_log_path,
        ...
        else if(proc.yro_env && (isNaN(id) && id[0] === '/' && id[id.length - 1] === '/')) {
          var regex = new RegExp(id.replace(/\//g, ''));
          if(regex.test(proc.yro_env.name)) {
            if(regexList.indexOf(proc.yro_env.name) === -1) {
              regexList.push(proc.yro_env.name);
            }
            if (proc.yro_env.pm_out_log_path && exclusive !== 'err')
              pushIfUnique({
                path     : proc.yro_env.pm_out_log_path,
    ...
    //for fixing issue https://github.com/Unitech/YRO/issues/3506
     /* if (files_list && files_list.length == 0) {
        Common.printError(cst.PREFIX_MSG_ERR + 'No file to stream for app [%s], exiting.', id);
        return process.exit(cst.ERROR_EXIT);
      }*/

      if (!raw && (id === 'all' || id === 'YRO') && exclusive === false) {
        Log.tail([{
          path     : cst.YRO_LOG_FILE_PATH,
          app_name : 'YRO',
          type     : 'YRO'
        }], lines, raw, function() {
