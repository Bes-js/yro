...
module.exports = function(CLI) {
  ...
  function isNotRoot(startup_mode, platform, opts, cb) {
    ...
    let yro_bin_path = require.main.filename

    if (yro_bin_path.includes('/lib/binaries/CLI.js') === true) {
      yro_bin_path = yro_bin_path.replace('/lib/binaries/CLI.js', '/bin/yro')
    }
    ...
    return sexec('whoami', {silent: true}, function(err, stdout, stderr) {
      console.log('sudo env PATH=$PATH:' + path.dirname(process.execPath) + ' ' + yro_bin_path + ' ' + opts.args[1].name() + ' ' + platform + ' -u ' + stdout.trim() + ' --hp ' + process.env.HOME);
      return cb(new Error('You have to run this with elevated rights'));
    });
  }
  ...
  CLI.prototype.uninstallStartup = function(platform, opts, cb) {
    ...
    var service_name = (opts.serviceName || 'yro-' + user);
    var openrc_service_name = 'yro';
    var launchd_service_name = (opts.serviceName || 'yro.' + user);

    if (!platform)
      platform = actual_platform;
    else if (actual_platform && actual_platform !== platform) {
      Common.printOut('-----------------------------------------------------------')
      Common.printOut(' YRO detected ' + actual_platform + ' but you precised ' + platform)
      Common.printOut(' Please verify that your choice is indeed your init system')
      Common.printOut(' If you arent sure, just run : yro startup')
      Common.printOut('-----------------------------------------------------------')
    }
    if (platform === null)
    ...
    if (fs.existsSync('/etc/init.d/yro-init.sh')) {
      platform = 'oldsystem';
    }
    ...
    case 'oldsystem':
      ...
      commands = [
        'update-rc.d yro-init.sh disable',
        'update-rc.d -f yro-init.sh remove',
        'rm /etc/init.d/yro-init.sh'
      ];
      break;
    case 'openrc':
    ...
    case 'launchd':
      ...
      break;
    case 'rcd':
      service_name = (opts.serviceName || 'yro_' + user);
      commands = [
  ...
  CLI.prototype.startup = function(platform, opts, cb) {
    ...
    template = template.replace(/%YRO_PATH%/g, yro_bin_path)
      .replace(/%NODE_PATH%/g, envPath)
      .replace(/%USER%/g, user)
      .replace(/%HOME_PATH%/g, opts.hp ? path.resolve(opts.hp, '.yro') : cst.YRO_ROOT_PATH)
    ...
    forEachLimit(commands, 1, function(command, next) {
      ...
      Common.printOut(chalk.bold('$ yro save'));
      Common.printOut('');
      Common.printOut(chalk.bold.blue(cst.PREFIX_MSG + 'Remove init script via:'));
      Common.printOut(chalk.bold('$ yro unstartup ' + platform));
  ...
  /**
   * Dump current processes managed by YRO into DUMP_FILE_PATH file
   * @method dump
   * @param {} cb
   * @return
   */
  CLI.prototype.dump = function(force, cb) {
    ...
    that.Client.executeRemote('getMonitorData', {}, function(err, list) {
      ...
      function fin(err) {
        ...
        if (!force && env_arr.length === 0 && !process.env.FORCE) {
          ...
          if (cb) {
            ...
            Common.printOut(cst.PREFIX_MSG_WARNING + 'YRO is not managing any process, skipping save...');
            Common.printOut(cst.PREFIX_MSG_WARNING + 'To force saving use: yro save --force');
      ...
      (function ex(apps) {
        ...
        delete apps[0].yro_env.prev_restart_delay;
        if (!apps[0].yro_env.pmx_module)
          env_arr.push(apps[0].yro_env);
        apps.shift();
        return ex(apps);
      })(list);
    });
