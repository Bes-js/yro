...
module.exports = function(CLI) {
  ...
  CLI.prototype._pull = function(opts, cb) {
    ...
    that.Client.getProcessByNameOrId(process_name, function (err, processes) {
      ...
      require('vizion').update({
        folder: proc.yro_env.versioning.repo_path
      }, function(err, meta) {
        if (err !== null) {
        ...
        if (meta.success === true) {
          getPostUpdateCmds(proc.yro_env.versioning.repo_path, process_name, function (command_list) {
            execCommands(proc.yro_env.versioning.repo_path, command_list, function(err, res) {
              if (err !== null) {
                printError(err);
  ...
  CLI.prototype.pullCommitId = function(process_name, commit_id, cb) {
    ...
    that.Client.getProcessByNameOrId(process_name, function (err, processes) {
      ...
      var proc = processes[0];
      if (proc.yro_env.versioning) {
        require('vizion').isUpToDate({folder: proc.yro_env.versioning.repo_path}, function(err, meta) {
          if (err !== null)
            return cb ? cb({msg:err}) : that.exitCli(cst.ERROR_EXIT);
  ...
  CLI.prototype.backward = function(process_name, cb) {
    ...
    that.Client.getProcessByNameOrId(process_name, function (err, processes) {
      ...
      if (proc.yro_env.versioning === undefined ||
      ...
      require('vizion').prev({
        ...
        getPostUpdateCmds(proc.yro_env.versioning.repo_path, process_name, function (command_list) {
          execCommands(proc.yro_env.versioning.repo_path, command_list, function(err, res) {
            if (err !== null) {
              require('vizion').next({folder: proc.yro_env.versioning.repo_path}, function(err2, meta2) {
                printError(err);
                return cb ? cb({msg: meta.output + err}) : that.exitCli(cst.ERROR_EXIT);
              });
              return false;
  ...
  CLI.prototype.forward = function(process_name, cb) {
    ...
    that.Client.getProcessByNameOrId(process_name, function (err, processes) {
      ...
      process_name = proc.name;
      if (proc.yro_env.versioning) {
        require('vizion').next({folder: proc.yro_env.versioning.repo_path}, function(err, meta) {
          if (err !== null)
            return cb ? cb({msg:err}) : that.exitCli(cst.ERROR_EXIT);
          if (meta.success === true) {
            getPostUpdateCmds(proc.yro_env.versioning.repo_path, process_name, function (command_list) {
              execCommands(proc.yro_env.versioning.repo_path, command_list, function(err, res) {
                if (err !== null)
                {
                  require('vizion').prev({folder: proc.yro_env.versioning.repo_path}, function(err2, meta2) {
                    printError(err);
                    return cb ? cb({msg:meta.output + err}) : that.exitCli(cst.ERROR_EXIT);
                  });
                }
  ...
  CLI.prototype._pullCommitId = function (opts, cb) {
    this.pullCommitId(opts.yro_name, opts.commit_id, cb);
  };
