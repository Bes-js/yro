    ...
    Configuration.set(key, value, function(err) {
      ...
      if (values && values.length > 1) {
        ...
        that.restart(app_name, {
          ...
          process.env.YRO_PROGRAMMATIC = 'false';
          if (!err)
...
function interactiveConfigEdit(cb) {
  UX.helpers.openEditor(cst.YRO_MODULE_CONF_FILE, function(err, data) {
    Common.printOut(chalk.bold('Module configuration (%s) edited.'), cst.YRO_MODULE_CONF_FILE);
    Common.printOut(chalk.bold('To take changes into account, please restart module related.'), cst.YRO_MODULE_CONF_FILE);
    if (err)
