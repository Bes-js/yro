    ...
    getLastLines(app.path, lines, function(output) {
      ...
      output.forEach(function(out) {
        ...
        if (app.type === 'out')
          ...
          process.stdout.write(chalk.blue(pad(DEFAULT_PADDING, 'YRO') + ' | '));
        console.log(out);
      });
      if (output.length)
  ...
  Client.launchBus(function(err, bus, socket) {
    ...
    socket.on('reconnect attempt', function() {
      if (global._auto_exit === true) {
        ...
        process.stdout.write(chalk.blue(pad(DEFAULT_PADDING, 'YRO') + ' | ') + '[[[ Target YRO killed. ]]]');
        process.exit(0);
      }
    });
    ...
    bus.on('log:*', function(type, packet) {
      ...
      lines.forEach(function(line) {
        ...
        else if (!raw && (id === 'all' || id === 'YRO'))
          process.stdout.write(chalk.blue(pad(' '.repeat(min_padding), 'YRO') + ' | '));
        if (highlight)
    ...
    bus.on('log:*', function(type, packet) {
      ...
      lines.forEach(function(line) {
        ...
        if (!raw) {
          ...
          if (packet.process.name === 'YRO')
            process.stdout.write('app=yro ');
          if (packet.process.name !== 'YRO')
