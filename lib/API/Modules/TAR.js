 ...
 *
 * YRO install http.tar.gz
 * YRO uninstall http
 *
 * - the first and only folder in the tarball must be called module (tar zcvf http module/)
 * - a package.json must be present with attribute "name", "version" and "YRO" to declare apps to run
 */

function install(YRO, module_filepath, opts, cb) {
  ...
  if (module_filepath.includes('http') === true) {
    ...
    return retrieveRemote(module_filepath, target_filepath, (err) => {
      ...
      installLocal(YRO, target_filepath, opts, cb)
    })
  }

  // Local install
  installLocal(YRO, module_filepath, opts, cb)
}
...
function installLocal(YRO, module_filepath, opts, cb) {
  ...
  getModuleName(module_filepath, function(err, module_name) {
    ...
    install_instance.on('close', function(code) {
      ...
      if (code == 0)
        return runInstall(YRO, install_path, module_name, opts, cb)
      return YRO.exitCli(1)
    });
...
function runInstall(YRO, target_path, module_name, opts, cb) {
  ...
  if (opts.install) {
    ...
    sexec(`cd ${target_path} ; yarn install`, {silent: false}, function(code) {
      // Start apps under "apps" or "YRO" attribute
      Common.logMod(`Starting ${target_path}`)
      YRO.start(conf, opts, function(err, data) {
        if (err) return cb(err)
...
function start(YRO, module_name, cb) {
 ...
 */
function uninstall(YRO, module_name, cb) {
  var module_path = path.join(cst.DEFAULT_MODULE_PATH, module_name);
  ...
  forEachLimit(apps, 1, (app, next) => {
    ...
    YRO._operate('deleteProcessId', app_name, () => {
...
function publish(YRO, folder, cb) {
  ...
  if (!pkg.YRO && !pkg.apps) throw new Error('Attribute apps should be present')
  ...
  packager(current_path, target_path, (err, res) => {
    ...
    var uri = `${YRO.yro_configuration.registry}/api/v1/modules`
