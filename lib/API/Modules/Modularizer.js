...
Modularizer.install = function (CLI, module_name, opts, cb) {
  ...
  if (LOCAL.INTERNAL_MODULES.hasOwnProperty(module_name)) {
    Common.logMod(`Adding dependency ${module_name} to YRO Runtime`);
...
/**
 * List modules based on modules present in ~/.yro/modules/ folder
 */
...
Modularizer.publish = function(YRO, folder, opts, cb) {
  if (opts.npm == true) {
    NPM.publish(opts, cb)
  }
  else {
    TAR.publish(YRO, folder, cb)
  }
};
