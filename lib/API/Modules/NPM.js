 ...
 * - Always ON, a module is always up along YRO, to stop it, you need to uninstall it
 * - Install a runnable module from NPM/Github/HTTP (require a package.json only)
 * - Some modules add internal YRO depencencies (like typescript, profiling...)
 ...
 * - Generate sample module via YRO module:generate <module_name>
 */

function localStart(YRO, opts, cb) {
  var proc_path = '',
      cmd  = '',
      conf = {};
  ...
  return StartModule(YRO, opts, function(err, dt) {
    ...
    sexec(cmd1, function(err) {
      ...
      sexec(cmd2, function(err) {
        ...
        sexec(cmd3, function(err) {
          ...
          Common.printOut('$ yro install . ');
          console.log('');

          Common.printOut('Module Log: ');
          Common.printOut('$ yro logs ' + module_name);
          console.log('');
          Common.printOut('Uninstall module: ');
          Common.printOut('$ yro uninstall ' + module_name);
          console.log('');
          Common.printOut('Force restart: ');
          Common.printOut('$ yro restart ' + module_name);
          return cb ?  cb() : false;
        });
      });
    });
    ...
    fs.writeFile(package_file, JSON.stringify(package_json, null, 2), function(err, data) {
      ...
      sexec('npm publish', function(code) {
        ...
        sexec('git add . ; git commit -m "' + package_json.version + '"; git push origin master', function(code) {
          ...
          Common.printOut(cst.PREFIX_MSG_MOD + 'Installable with yro install %s', package_json.name);
  ...
  function finalizeInstall(code) {
    ...
    Configuration.set(cst.MODULE_CONF_PREFIX + ':' + canonic_module_name, {
      ...
      StartModule(CLI, opts, function(err, dt) {
        ...
        if (process.env.YRO_PROGRAMMATIC === 'true')
          return cb(null, dt);

        CLI.conf(canonic_module_name, function() {
          Common.printOut(cst.PREFIX_MSG_MOD + 'Module successfully installed and launched');
          Common.printOut(cst.PREFIX_MSG_MOD + 'Checkout module options: `$ yro conf`');
          return cb(null, dt);
        });
      });
    });
...
function start(YRO, modules, module_name, cb) {
  ...
  StartModule(YRO, opts, function(err, dt) {
...
function StartModule(CLI, opts, cb) {
   ...
   * 1- *apps* field (default YRO json configuration)
   * 2- *bin* field
   * 3- *main* field
   */
  if (!package_json.apps && !package_json.yro) {
    package_json.apps = {};
    ...
    if (opts.safe) {
      ...
      return setTimeout(function() {
        CLI.describe(package_json.name, function(err, apps) {
          if (err || apps[0].yro_env.restart_time > 2) {
