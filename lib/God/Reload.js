...
function softReload(God, id, cb) {
  ...
  // Deep copy
  var new_env = Utility.clone(old_worker.yro_env);
  ...
  old_worker.yro_env.pm_id = t_key;
    ...
    var softCleanDeleteProcess = function () {
      ...
      try {
        if (old_worker.state != 'dead' && old_worker.state != 'disconnected')
          ...
          clearTimeout(timer);
          console.error('Worker %d is already disconnected', old_worker.yro_env.pm_id);
          return God.deleteProcessId(t_key, cb);
        }
      } catch(e) {
        clearTimeout(timer);
...
function hardReload(God, id, wait_msg, cb) {
  ...
  var listener = function (packet) {
    if (packet.raw === 'ready' &&
        packet.process.name === old_worker.yro_env.name &&
...
module.exports = function(God) {
  ...
  God.softReloadProcessId = function(opts, cb) {
    ...
    if (God.clusters_db[id].yro_env.status == cst.ONLINE_STATUS &&
        God.clusters_db[id].yro_env.exec_mode == 'cluster_mode' &&
        !God.clusters_db[id].yro_env.wait_ready) {

      Utility.extend(God.clusters_db[id].yro_env.env, opts.env);
  ...
  God.reloadProcessId = function(opts, cb) {
    ...
    if (!(id in God.clusters_db))
      return cb(new Error('YRO ID unknown'));
    ...
    if (God.clusters_db[id].yro_env.status == cst.ONLINE_STATUS &&
      ...
      var wait_msg = God.clusters_db[id].yro_env.wait_ready ? 'ready' : 'listening';
