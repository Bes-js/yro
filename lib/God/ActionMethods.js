...
var debug         = require('debug')('yro:ActionMethod');
...
module.exports = function(God) {
  ...
  God.dumpProcessList = function(cb) {
    ...
    // unless user explicitely did `YRO dump`.
    // This often happens when YRO crashed, we don't want to override
    // the dump file with an empty list of process.
    if (!apps[0]) {
      debug('[YRO] Did not override dump file because list of processes is empty');
    ...
    function fin(err) {
      ...
      if (process_list.length === 0) {
        ...
        // fix : if no dump file, no process, only module and after YRO update
    ...
    function saveProc(apps) {
      if (!apps[0])
        return fin(null);
      delete apps[0].yro_env.instances;
      delete apps[0].yro_env.pm_id;
      // Do not dump modules
      if (!apps[0].yro_env.pmx_module)
        process_list.push(apps[0].yro_env);
      apps.shift();
  ...
  /**
   * Description
   * @method notifyKillYRO
   */
  God.notifyKillYRO = function() {
    God.yro_being_killed = true;
  };
  ...
  God.duplicateProcessId = function(id, cb) {
    ...
    if (!God.clusters_db[id] || !God.clusters_db[id].yro_env)
      return cb(God.logAndGenerateError('Error when getting proc || proc.yro_env'), {});

    var proc = Utility.clone(God.clusters_db[id].yro_env);
  ...
  God.startProcessId = function(id, cb) {
    ...
    if (proc.yro_env.status == cst.ONLINE_STATUS)
      return cb(God.logAndGenerateError('process already online'), {});
    if (proc.yro_env.status == cst.LAUNCHING_STATUS)
    ...
    return God.executeApp(God.clusters_db[id].yro_env, function(err, proc) {
  ...
  God.stopProcessId = function(id, cb) {
    ...
    clearTimeout(proc.yro_env.restart_task);

    if (proc.yro_env.status == cst.STOPPED_STATUS) {
      proc.process.pid = 0;
    ...
    console.log('Stopping app:%s id:%s', proc.yro_env.name, proc.yro_env.pm_id);
    proc.yro_env.status = cst.STOPPING_STATUS;

    if (!proc.process.pid) {
      console.error('app=%s id=%d does not have a pid', proc.yro_env.name, proc.yro_env.pm_id);
      proc.yro_env.status = cst.STOPPED_STATUS;
      return cb(null, { error : true, message : 'could not kill process w/o pid'});
    }

    God.killProcess(proc.process.pid, proc.yro_env, function(err) {
      proc.yro_env.status = cst.STOPPED_STATUS;
  ...
  God.resetMetaProcessId = function(id, cb) {
    ...
    God.clusters_db[id].yro_env.created_at = Utility.getDate();
    God.clusters_db[id].yro_env.unstable_restarts = 0;
    God.clusters_db[id].yro_env.restart_time = 0;
  ...
  God.restartProcessId = function(opts, cb) {
     ...
     */
    Utility.extend(proc.yro_env.env, env);
    Utility.extendExtraConfig(proc, opts);

    if (God.yro_being_killed) {
      return cb(God.logAndGenerateError('[RestartProcessId] YRO is being killed, stopping restart procedure...'));
    }
    if (proc.yro_env.status === cst.ONLINE_STATUS || proc.yro_env.status === cst.LAUNCHING_STATUS) {
  ...
  God.restartProcessName = function(name, cb) {
    ...
    eachLimit(processes, cst.CONCURRENT_ACTIONS, function(proc, next) {
      if (God.yro_being_killed)
        return next('[Watch] YRO is being killed, stopping restart procedure...');
      if (proc.yro_env.status === cst.ONLINE_STATUS)
        return God.restartProcessId({id:proc.yro_env.pm_id}, next);
      else if (proc.yro_env.status !== cst.STOPPING_STATUS
               && proc.yro_env.status !== cst.LAUNCHING_STATUS)
        return God.startProcessId(proc.yro_env.pm_id, next);
      else
  ...
  God.stopWatch = function(method, value, fn) {
    ...
    if (method == 'stopAll' || method == 'deleteAll') {
      ...
      processes.forEach(function(proc) {
        God.clusters_db[proc.pm_id].yro_env.watch = false;
        God.watch.disable(proc.yro_env);
      });
      ...
      if (env) {
        God.watch.disable(env.yro_env);
        env.yro_env.watch = false;
      }
    }
  ...
  God.toggleWatch = function(method, value, fn) {
    ...
    if (env) {
      env.yro_env.watch = !env.yro_env.watch;
      if (env.yro_env.watch)
        God.watch.enable(env.yro_env);
      else
        God.watch.disable(env.yro_env);
    }
  ...
  God.startWatch = function(method, value, fn) {
    ...
    if (env) {
      ...
      God.watch.enable(env.yro_env);
      //env.yro_env.env.watch = true;
      env.yro_env.watch = true;
    }
  ...
  God.reloadLogs = function(opts, cb) {
    ...
    processIds.forEach(function (id) {
      ...
      if (cluster && cluster.yro_env) {
        // Cluster mode
        if (cluster.send && cluster.yro_env.exec_mode == 'cluster_mode') {
          try {
            cluster.send({
              type:'log:reload'
            });
  ...
  God.sendLineToStdin = function(packet, cb) {
    ...
    if (proc.yro_env.exec_mode == 'cluster_mode')
  ...
  God.msgProcess = function(cmd, cb) {
    if ('id' in cmd) {
      ...
      proc.yro_env.axm_actions.forEach(function(action) {
      ...
      if (action_exist == false) {
        return cb(God.logAndGenerateError('Action doesn\'t exist ' + cmd.msg + ' for ' + proc.yro_env.name), {});
      }
      ...
      (function ex(arr) {
        ...
        var proc_env = God.clusters_db[id].yro_env;
  ...
  God.monitor = function Monitor(pm_id, cb) {
    if (!God.clusters_db[pm_id] || !God.clusters_db[pm_id].yro_env)
      return cb(new Error('Unknown pm_id'));

    God.clusters_db[pm_id].yro_env._km_monitored = true;
  ...
  God.unmonitor = function Monitor(pm_id, cb) {
    ...
    God.clusters_db[pm_id].yro_env._km_monitored = false;
    return cb(null, { success : true, pm_id : pm_id });
  }
...
function filterBadProcess(pro) {
  if (pro.yro_env.status !== cst.ONLINE_STATUS) {
    return false;
  }

  if (pro.yro_env.axm_options && pro.yro_env.axm_options.pid) {
    if (isNaN(pro.yro_env.axm_options.pid))  {
      return false;
    }
  }
...
function getProcessId(pro) {
  ...
  if (pro.yro_env.axm_options && pro.yro_env.axm_options.pid) {
    pid = pro.yro_env.axm_options.pid;
  }
