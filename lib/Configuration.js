...
Configuration.set = function(key, value, cb) {
  fs.readFile(cst.YRO_MODULE_CONF_FILE, function(err, data) {
    ...
    fs.writeFile(cst.YRO_MODULE_CONF_FILE, serializeConfiguration(json_conf), function(err, data) {
      if (err) return cb(err);
...
Configuration.setSyncIfNotExist = function(key, value) {
  try {
    var conf = JSON.parse(fs.readFileSync(cst.YRO_MODULE_CONF_FILE));
  } catch(e) {
    return null;
  }
...
Configuration.setSync = function(key, value) {
  try {
    var data = fs.readFileSync(cst.YRO_MODULE_CONF_FILE);
  } catch(e) {
    return null;
  }
  ...
  try {
    fs.writeFileSync(cst.YRO_MODULE_CONF_FILE, serializeConfiguration(json_conf));
    return json_conf;
  } catch(e) {
...
Configuration.getAllSync = function() {
  try {
    return JSON.parse(fs.readFileSync(cst.YRO_MODULE_CONF_FILE));
  } catch(e) {
