...
const debug       = require('debug')('yro:cli');
 ...
 * can be aliased to YRO
 *
 * To use it when YRO is installed as a module:
 *
 * var YRO = require('YRO');
 *
 * var YRO = YRO(<opts>);
 ...
 *
 * @param {Object}  opts
 * @param {String}  [opts.cwd=<current>]         override YRO cwd for starting scripts
 * @param {String}  [opts.YRO_home=[<paths.js>]] YRO directory for log, pids, socket files
 * @param {Boolean} [opts.independent=false]     unique YRO instance (random YRO_home)
 * @param {Boolean} [opts.daemon_mode=true]      should be called in the same process or not
 * @param {String}  [opts.public_key=null]       YRO plus bucket public key
 * @param {String}  [opts.secret_key=null]       YRO plus bucket secret key
 * @param {String}  [opts.machine_name=null]     YRO plus instance name
 */
class API {
  ...
  constructor (opts) {
    ...
    this.YRO_home = conf.YRO_ROOT_PATH;
     ...
     * YRO HOME resolution
     */
    if (opts.YRO_home && opts.independent == true)
      throw new Error('You cannot set a YRO_home and independent instance in same time');

    if (opts.YRO_home) {
      // Override default conf file
      this.YRO_home = opts.YRO_home;
      conf = Object.assign(conf, path_structure(this.YRO_home));
    }
    else if (opts.independent == true && conf.IS_WINDOWS === false) {
      // Create an unique YRO instance
      const crypto = require('crypto');
      var random_file = crypto.randomBytes(8).toString('hex');
      this.YRO_home = path.join('/tmp', random_file);
    ...
    this.yro_configuration = Configuration.getSync('YRO') || {}
   ...
   *
   * @param {Function} cb callback once YRO is ready for commands
   */
  connect (noDaemon, cb) {
    var that = this;
    ...
    if (typeof(cb) == 'undefined') {
      ...
      // Backward compatibility with YRO 1.x
    ...
    this.Client.start(function(err, meta) {
      ...
      if (meta.new_YRO_instance == false && that.daemon_mode === true)
        return cb(err, meta);

      that.launchSysMonitoring(() => {})
      // If new YRO instance has been popped
      // Launch all modules
  ...
  /**
   * Usefull when custom YRO created with independent flag set to true
   * This will cleanup the newly created instance
   * by removing folder, killing YRO and so on
   *
  ...
  destroy (cb) {
    ...
    this.killDaemon(function() {
      var cmd = 'rm -rf ' + that.YRO_home;
      var test_path = path.join(that.YRO_home, 'module_conf.json');
      var test_path_2 = path.join(that.YRO_home, 'YRO.pid');

      if (that.YRO_home.indexOf('.YRO') > -1)
        return cb(new Error('Destroy is not a allowed method on .YRO'));

      fs.access(test_path, fs.R_OK, function(err) {
        if (err) return cb(err);
        debug('Deleting temporary folder %s', that.YRO_home);
        sexec(cmd, cb);
      });
    });
  }
   ...
   * Launch modules
   *
   * @param {Function} cb callback once YRO has launched modules
   */
  launchModules (cb) {
  ...
  exitCli (code) {
    ...
    // Do nothing if YRO called programmatically (also in speedlist)
    if (conf.YRO_PROGRAMMATIC && process.env.YRO_USAGE != 'CLI') return false;
...
////////////////////////////
  ...
  /**
   * Update daemonized YRO Daemon
   *
   * @param {Function} cb callback when YRO has been upgraded
   */
  update (cb) {
    var that = this;

    Common.printOut('Be sure to have the latest version by doing `npm install yro@latest -g` before doing this procedure.');

    // Dump YRO processes
    that.Client.executeRemote('notifyKillYRO', {}, function() {});
          ...
          that.Client.launchDaemon({interactor:false}, function(err, child) {
            that.Client.launchRPC(function() {
              that.resurrect(function() {
                Common.printOut(chalk.blue.bold('>>>>>>>>>> YRO updated'));
                that.launchSysMonitoring(() => {})
                that.launchAll(that, function() {
                  KMDaemon.launchAndInteract(that._conf, {
                    YRO_version: pkg.version
   ...
   */
  killDaemon (cb) {
    process.env.YRO_STATUS = 'stopping'
    ...
    that._operate('deleteProcessId', 'all', function(err, list) {
      ...
      process.env.YRO_SILENT = 'false';
      

        that.Client.killDaemon(function(err, res) {
          if (err) Common.printError(err);
          Common.printOut(conf.PREFIX_MSG + '[v] YRO Daemon Stopped');
  ...
  _startScript (script, opts, cb) {
     ...
     */
    if (app_conf.write) {
      var dst_path = path.join(process.env.PWD || process.cwd(), app_conf.name + '-YRO.json');
    ...
    function restartExistingProcessPathOrStartNew(cb) {
      that.Client.executeRemote('getMonitorData', {}, function(err, procs) {
        ...
        procs.forEach(function(proc) {
          if (proc.yro_env.pm_exec_path == full_path &&
              proc.yro_env.name == app_conf.name)
            managed_script = proc;
        });

        if (managed_script &&
          (managed_script.yro_env.status == conf.STOPPED_STATUS ||
            managed_script.yro_env.status == conf.STOPPING_STATUS ||
            managed_script.yro_env.status == conf.ERRORED_STATUS)) {
          // Restart process if stopped
          var app_name = managed_script.yro_env.name;
        ...
        // Set YRO HOME in case of child process using YRO API
        resolved_paths.env['YRO_HOME'] = that.YRO_home;
  ...
  _startJson (file, opts, action, pipe, cb) {
    ...
    process.env.YRO_JSON_PROCESSING = true;
    ...
    staticConf.forEach(function(serve) {
      appConf.push({
        ...
        env: {
          YRO_SERVE_PORT: serve.port,
          YRO_SERVE_HOST: serve.host,
          YRO_SERVE_PATH: serve.path,
          YRO_SERVE_SPA: serve.spa,
          YRO_SERVE_DIRECTORY: serve.directory,
          YRO_SERVE_BASIC_AUTH: serve.basic_auth !== undefined,
          YRO_SERVE_BASIC_AUTH_USERNAME: serve.basic_auth ? serve.basic_auth.username : null,
          YRO_SERVE_BASIC_AUTH_PASSWORD: serve.basic_auth ? serve.basic_auth.password : null,
          YRO_SERVE_MONITOR: serve.monitor
        }
      });
    });
    ...
    function startApps(app_name_to_start, cb) {
      ...
      eachLimit(apps_to_start, conf.CONCURRENT_ACTIONS, function(app, next) {
        ...
        that.Client.executeRemote('prepare', resolved_paths, function(err, data) {
          ...
          Common.printOut(conf.PREFIX_MSG + 'App [%s] launched (%d instances)', data[0].yro_env.name, data.length);
  ...
  /**
   * Main function to operate with YRO daemon
   *
  ...
  _operate (action_name, process_name, envs, cb) {
    ...
    if (!process.env.YRO_JSON_PROCESSING || envs.commands) {
      ...
      eachLimit(ids, concurrent_actions, function(id, next) {
        ...
        that.Client.executeRemote(action_name, opts, function(err, res) {
          ...
          res.forEach(function(proc) {
            Common.printOut(conf.PREFIX_MSG + '[%s](%d) \u2713', proc.yro_env ? proc.yro_env.name : process_name, id);

            if (action_name == 'stopProcessId' && proc.yro_env && proc.yro_env.cron_restart) {
              Common.warn(`App ${chalk.bold(proc.yro_env.name)} stopped but CRON RESTART is still UP ${proc.yro_env.cron_restart}`)
            }

            if (!proc.yro_env) return false;

            ret.push({
              name         : proc.yro_env.name,
              namespace: proc.yro_env.namespace,
              pm_id        : proc.yro_env.pm_id,
              status       : proc.yro_env.status,
              restart_time : proc.yro_env.restart_time,
              yro_env : {
                name         : proc.yro_env.name,
                namespace: proc.yro_env.namespace,
                pm_id        : proc.yro_env.pm_id,
    ...
    if (process_name == 'all') {
      ...
      if (process.env.YRO_STATUS == 'stopping')
        that.Client.getAllProcessId(function(err, ids) {
          reoperate(err, ids)
        });
      else
    ...
    // operate using regex
      ...
      that.Client.executeRemote('getMonitorData', {}, function(err, list) {
        ...
        list.forEach(function(proc) {
          if (regex.test(proc.yro_env.name)) {
      ...
      if (that.yro_configuration.docker == "true" ||
          that.yro_configuration.docker == true) {
  ...
  _handleAttributeUpdate (opts) {
    ...
    if (process.env.YRO_DEEP_MONITORING)
  ...
  speedList (code, apps_acted) {
    ...
    if (apps_acted && apps_acted.length > 0) {
      apps_acted.forEach(proc => {
        acted.push(proc.yro_env ? proc.yro_env.pm_id : proc.pm_id)
      })
    }

    // Do nothing if YRO called programmatically and not called from CLI (also in exitCli)
    if ((conf.YRO_PROGRAMMATIC && process.env.YRO_USAGE != 'CLI'))
      return false;
    ...
    function doList(err, list) {
      ...
      if (process.stdout.isTTY === false) {
        ...
        if (that.gl_interact_infos) {
          var dashboard_url = `https://app.YRO.io/#/r/${that.gl_interact_infos.public_key}`
          ...
          Common.printOut('%s YRO+ activated | Instance Name: %s | Dash: %s',
                          chalk.green.bold('⇆'),
                          chalk.bold(that.gl_interact_infos.machine_name),
                          chalk.bold(dashboard_url))
        }
        UX.list(list, commander);
        //Common.printOut(chalk.white.italic(' Use `YRO show <id|name>` to get more details about an app'));
      }

      if (that.Client.daemon_mode == false) {
        Common.printOut('[--no-daemon] Continue to stream logs');
        Common.printOut('[--no-daemon] Exit on target YRO exit pid=' + fs.readFileSync(conf.YRO_PID_FILE_PATH).toString());
      ...
      // if (process.stdout.isTTY) if looking for start logs
        ...
        // setTimeout(() => {
        //   Common.info(`Log streaming exited automatically, run 'YRO logs' to continue watching logs`)
        //   return that.exitCli(code ? code : conf.SUCCESS_EXIT);
        // }, 10000)
  ...
  scale (app_name, number, cb) {
    ...
    function addProcs(proc, value, cb) {
      (function ex(proc, number) {
        ...
        that.Client.executeRemote('duplicateProcessId', proc.yro_env.pm_id, ex.bind(this, proc, number));
      })(proc, number);
    }
    ...
    function rmProcs(procs, value, cb) {
      ...
      (function ex(procs, number) {
        if (number++ === 0) return cb();
        that._operate('deleteProcessId', procs[i++].yro_env.pm_id, ex.bind(this, procs, number));
      })(procs, number);
    }
  ...
  describe (YRO_id, cb) {
    ...
    that.Client.executeRemote('getMonitorData', {}, function(err, list) {
      ...
      list.forEach(function(proc) {
        if ((!isNaN(YRO_id)    && proc.pm_id == YRO_id) ||
          (typeof(YRO_id) === 'string' && proc.name  == YRO_id)) {
          found_proc.push(proc);
        }
      });

      if (found_proc.length === 0) {
        Common.printError(conf.PREFIX_MSG_WARNING + '%s doesn\'t exist', YRO_id);
  ...
  /**
   * API method to perform a deep update of YRO
   * @method deepUpdate
   */
  deepUpdate (cb) {
    var that = this;

    Common.printOut(conf.PREFIX_MSG + 'Updating YRO...');

    var child = sexec("npm i -g YRO@latest; YRO update");

    child.stdout.on('end', function() {
      Common.printOut(conf.PREFIX_MSG + 'YRO successfully updated');
