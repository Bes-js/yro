...
var Utility = module.exports = {
  ...
  extendExtraConfig : function(proc, opts) {
    if (opts.env && opts.env.current_conf) {
      ...
      Utility.extendMix(proc.yro_env, opts.env.current_conf);
  ...
  formatCLU : function(process) {
    ...
    var obj = Utility.clone(process.yro_env);
    delete obj.env;
  ...
  overrideConsole : function(bus) {
    if (cst.YRO_LOG_DATE_FORMAT && typeof cst.YRO_LOG_DATE_FORMAT == 'string') {
      // Generate timestamp prefix
      function timestamp(){
        return `${dayjs(Date.now()).format(cst.YRO_LOG_DATE_FORMAT)}:`;
      }
      ...
      hacks.forEach(function(k){
        console[k] = function(){
          ...
          arguments[0] && (arguments[0] = timestamp() + ' YRO ' + k + ': ' + arguments[0]);
  ...
  startLogging : function(stds, callback) {
    ...
    //     })(path.resolve(cst.YRO_ROOT_PATH, n));
    //   });
    // } catch(err) {
    ...
    (function createWS(io){
      ...
      // compatible with `YRO reloadLogs`
   ...
   * @example Always returns 'YRO-slack' for inputs 'ma-zal/YRO-slack', 'ma-zal/YRO-slack#own-branch',
   *          'YRO-slack-1.0.0.tgz' or 'YRO-slack@1.0.0'.
   */
