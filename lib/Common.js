...
Common.lockReload = function() {
  try {
    var t1 = fs.readFileSync(cst.YRO_RELOAD_LOCKFILE).toString();
  ...
  try {
    ...
    fs.writeFileSync(cst.YRO_RELOAD_LOCKFILE, dayjs().valueOf().toString());
    return 0;
  } catch(e) {
...
Common.unlockReload = function() {
  try {
    fs.writeFileSync(cst.YRO_RELOAD_LOCKFILE, '');
  } catch(e) {
    console.error(e.message || e);
  }
};
...
Common.prepareAppConf = function(opts, app) {
  ...
  /**
   * Do not copy internal YRO environment variables if acting on process
   * is made from a programmatic script started by YRO or if a pm_id is present in env
   */
  if (cst.YRO_PROGRAMMATIC || process.env.pm_id)
...
Common.errMod = function(msg) {
  if (process.env.YRO_SILENT || process.env.YRO_PROGRAMMATIC === 'true') return false;
...
Common.printOut = function() {
  if (process.env.YRO_SILENT === 'true' || process.env.YRO_PROGRAMMATIC === 'true') return false;
...
Common.safeExtend = function(origin, add){
  ...
  //Ignore YRO's set environment variables from the nested env
...
Common.verifyConfs = function(appConfs) {
  ...
  for (var i = 0; i < appConfs.length; i++) {
    ...
    /**
     * If command is like YRO start "python xx.py --ok"
     ...
     * comes from YRO --uid <> --gid <> or --user
     */
    if (app.uid || app.gid || app.user) {
      ...
      if (process.env.NODE_ENV != 'test' && process.getuid && process.getuid() !== 0) {
        Common.printError(cst.PREFIX_MSG_ERR + 'To use --uid and --gid please run YRO as root');
        return new Error('To use UID and GID please run YRO as root');
      }
...
Common.getCurrentUsername = function(){
  ...
  if (os.userInfo) {
    try {
      ...
      // https://github.com/Unitech/YRO/issues/3184
    }
  }
