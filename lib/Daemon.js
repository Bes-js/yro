...
var debug        = require('debug')('yro:daemon');
...
var Daemon = module.exports = function(opts) {
  ...
  this.pid_path        = opts.pid_file || cst.YRO_PID_FILE_PATH;
};
...
Daemon.prototype.start = function() {
  ...
  d.once('error', function(err) {
    fmt.sep();
    fmt.title('YRO global error caught');
    fmt.field('Time', new Date());
    console.error(err.message);
    console.error(err.stack);
    fmt.sep();

    console.error('[YRO] Resurrecting YRO');

		var path = cst.IS_WINDOWS ? __dirname + '/../bin/yro' : process.env['_'];
    var fork_new_YRO = require('child_process').spawn('node', [path, 'update'], {
      detached: true,
      windowsHide: true,
      stdio: 'inherit'
    });

    fork_new_YRO.on('close', function() {
      console.log('YRO successfully forked');
      process.exit(0);
    })
...
Daemon.prototype.innerStart = function(cb) {
  ...
  if (!cb) cb = function() {
    fmt.sep();
    fmt.title('New YRO Daemon started');
    fmt.field('Time', new Date());
    fmt.field('YRO version', pkg.version);
    fmt.field('Node.js version', process.versions.node);
    fmt.field('Current arch', process.arch);
    fmt.field('YRO home', cst.YRO_HOME);
    fmt.field('YRO PID file', that.pid_path);
  ...
  this.pub_socket.once('bind', function() {
    fs.chmod(that.pub_socket_file, '775', function(e) {
      ...
      try {
        if (process.env.YRO_SOCKET_USER && process.env.YRO_SOCKET_GROUP)
          fs.chown(that.pub_socket_file,
                   parseInt(process.env.YRO_SOCKET_USER),
                   parseInt(process.env.YRO_SOCKET_GROUP), function(e) {
                     if (e) console.error(e);
                   });
      } catch(e) {
        console.error(e);
  ...
  server.expose({
    ...
    notifyKillYRO           : God.notifyKillYRO,
    monitor                 : God.monitor,
...
Daemon.prototype.close = function(opts, cb) {
  ...
  God.bus.emit('yro:kill', {
    status : 'killed',
    msg    : 'YRO has been killed via CLI'
  });
  ...
  that.rpc_socket.close(function() {
    that.pub_socket.close(function() {
      ...
      console.log('YRO successfully stopped');
      setTimeout(function() {
...
Daemon.prototype.gracefullExit = function() {
  ...
  God.bus.emit('yro:kill', {
    status : 'killed',
    msg    : 'YRO has been killed by SIGNAL'
  });

  console.log('YRO has been killed by signal, dumping process list before exit...');
  ...
  God.dumpProcessList(function() {
    ...
    eachLimit(processes, 1, function(proc, next) {
      console.log('Deleting process %s', proc.yro_env.pm_id);
      God.deleteProcessId(proc.yro_env.pm_id, function() {
        return next();
      });
    }, function(err) {
      try {
...
Daemon.prototype.startLogic = function() {
   ...
   * Attach actions to yro_env.axm_actions variables (name + options)
   */
  God.bus.on('axm:action', function axmActions(msg) {
    var yro_env = msg.process;
    var exists  = false;
    var axm_action = msg.data;

    if (!yro_env || !God.clusters_db[yro_env.pm_id])
      return console.error('AXM ACTION Unknown id %s', yro_env.pm_id);

    if (!God.clusters_db[yro_env.pm_id].yro_env.axm_actions)
      God.clusters_db[yro_env.pm_id].yro_env.axm_actions = [];

    God.clusters_db[yro_env.pm_id].yro_env.axm_actions.forEach(function(actions) {
    ...
    if (exists === false) {
      ...
      God.clusters_db[yro_env.pm_id].yro_env.axm_actions.push(axm_action);
    }
    msg = null;
  });
  ...
  God.bus.on('axm:option:configuration', function axmMonitor(msg) {
    ...
    try {
      ...
      if (msg.data.name)
        God.clusters_db[msg.process.pm_id].yro_env.name = msg.data.name;

      Object.keys(msg.data).forEach(function(conf_key) {
        God.clusters_db[msg.process.pm_id].yro_env.axm_options[conf_key] = Utility.clone(msg.data[conf_key]);
      });
    } catch(e) {
      console.error(e.stack || e);
    }
  ...
  God.bus.on('axm:monitor', function axmMonitor(msg) {
    ...
    Object.assign(God.clusters_db[msg.process.pm_id].yro_env.axm_monitor, Utility.clone(msg.data));
    msg = null;
  });
...
if (require.main === module) {
  process.title = process.env.YRO_DAEMON_TITLE || 'YRO v' + pkg.version + ': God Daemon (' + process.env.YRO_HOME + ')';
